...
class StartupAndDockerTests(unittest.TestCase):
    ...
    def test_docker_runtime(self):
        ...
        bootstrap = (root / "bootstrap_v2.py").read_text(encoding="utf-8")
        requirements = (root / "requirements.txt").read_text(encoding="utf-8")
        self.assertIn("ffmpeg", docker)
        self.assertIn("node:22", docker)
        self.assertNotIn("deno", docker.lower())
        self.assertIn("bgutil-ytdlp-pot-provider", docker)
        self.assertIn("/ping", docker)
        self.assertIn("bootstrap_v2.py", docker)
        self.assertIn("telegram_fit_patch", bootstrap)
        self.assertIn("youtube_auth_patch", bootstrap)
